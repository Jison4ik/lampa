...
function init(){
    ...
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_title',type:'title'},
        field:{name:'Кастомный фон (Grind)'}
    });
    
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_bg_enabled',type:'trigger',default:false},
        field:{name:'Включить кастомный фон',description:'Показывать изображение вместо серого фона'},
        onChange:function(v){if(v)applyBackground();else removeBackground();}
    });
    
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_bg_url',type:'input',default:''},
        field:{name:'URL изображения',description:'Ссылка на картинку',placeholder:'https://example.com/image.jpg'},
        onChange:function(){if(Lampa.Storage.get('grind_bg_enabled'))applyBackground();}
    });
    
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_bg_mode',type:'select',values:{'cover':'Заполнить','contain':'Вместить','stretch':'Растянуть'},default:'cover'},
        field:{name:'Режим отображения',description:'Как заполнять экран'},
        onChange:function(){if(Lampa.Storage.get('grind_bg_enabled'))applyBackground();}
    });
    
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_bg_blur',type:'select',values:{'0':'Без размытия','5':'Легкое (5px)','10':'Среднее (10px)','15':'Сильное (15px)'},default:'5'},
        field:{name:'Размытие',description:'Blur эффект'},
        onChange:function(){if(Lampa.Storage.get('grind_bg_enabled'))applyBackground();}
    });
    
    Lampa.SettingsApi.addParam({
        component:'interface',
        param:{name:'grind_bg_overlay',type:'select',values:{'0':'Без затемнения','0.3':'Легкое (30%)','0.5':'Среднее (50%)','0.7':'Сильное (70%)'},default:'0.5'},
        field:{name:'Затемнение',description:'Темный слой'},
        onChange:function(){if(Lampa.Storage.get('grind_bg_enabled'))applyBackground();}
    });
